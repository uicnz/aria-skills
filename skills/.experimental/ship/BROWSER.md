...
This document covers the command reference and internals of ship's headless browser.
...
| Visual     | `screenshot [--viewport] [--clip x,y,w,h] [sel\|@ref] [path]`, `pdf`, `responsive`               | See what Aria sees                       |
...
ship's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.ship/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
- Console: `.ship/browse-console.log`
- Network: `.ship/browse-network.log`
- Dialog: `.ship/browse-dialog.log`
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.ship/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace         | State file                          | Port                 |
| ----------------- | ----------------------------------- | -------------------- |
| `/code/project-a` | `/code/project-a/.ship/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.ship/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE`    | `.ship/browse.json`    | Path to state file (CLI passes to server)       |
...
| **ship browse** | **~3s**    | **~100-200ms**   | **0 tokens** (plain text stdout) |

The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. ship burns zero.
...
- **Unnecessary abstraction**: Aria already has a Bash tool. A CLI that prints to stdout is the simplest possible interface.

ship skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
| File                                  | Role                                                                                                                 |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `browse/src/cli.ts`                   | Entry point. Reads `.ship/browse.json`, sends HTTP to the server, prints response.                                   |
...
The active skill lives at `~/.aria/skills/ship/`. After making changes:

1. Push your branch
2. Pull in the skill directory: `cd ~/.aria/skills/ship && git pull`
3. Rebuild: `cd ~/.aria/skills/ship && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.aria/skills/ship/browse/dist/browse`
