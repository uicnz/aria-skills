/**
 * Aria CLI subprocess runner for skill E2E testing.
 *
 * Spawns `aria -p` as a completely independent process (not via Agent SDK),
 * so it works inside Aria sessions. Pipes prompt via stdin, streams
...
const SHIP_DEV_DIR = path.join(os.homedir(), ".ship-dev");
const HEARTBEAT_PATH = path.join(SHIP_DEV_DIR, "e2e-live.json");
...
export async function runSkillTest(options: {
	...
	if (runId) {
		try {
			runDir = path.join(SHIP_DEV_DIR, "e2e-runs", runId);
	...
	// Spawn aria -p with streaming NDJSON output. Prompt piped via stdin to
	...
	const proc = Bun.spawn(
		[
			"sh",
			"-c",
			`cat "${promptFile}" | aria ${args.map((a) => `"${a}"`).join(" ")}`,
		],
		{
	...
	if (resultLine) {
		if (resultLine.is_error) {
			// aria -p can return subtype=success with is_error=true (e.g. API connection failure)
	...
	if (browseErrors.length > 0 || exitReason !== "success") {
		try {
			const failureDir =
				runDir || path.join(workingDirectory, ".ship", "test-transcripts");
