...
describe("session-runner observability", () => {
	test("1: sanitizeTestName strips slashes and leading dashes", () => {
		expect(sanitizeTestName("/plan-review-founder")).toBe(
			"plan-review-founder",
		);
	...
	test("2: heartbeat file path uses ~/.ship-dev/e2e-live.json", () => {
		// Just verify the constant is correct — actual write is tested by E2E
		const expected = path.join(os.homedir(), ".ship-dev", "e2e-live.json");
...
describe("eval-watch dashboard", () => {
	test("9: renderDashboard shows completed tests and current test", () => {
		const heartbeat: HeartbeatData = {
			...
			currentTest: "plan-review-founder",
			status: "running",
			turn: 4,
			toolCount: 3,
		...
		expect(output).toContain("plan-review-founder");
