...
describe("SKILL.md command validation", () => {
	...
	test("all $B commands in plan-review-design/SKILL.md are valid browse commands", () => {
		const skill = path.join(ROOT, "plan-review-design", "SKILL.md");
		if (!fs.existsSync(skill)) return;
		const result = validateSkill(skill);
		expect(result.invalid).toHaveLength(0);
	});

	test("all snapshot flags in plan-review-design/SKILL.md are valid", () => {
		const skill = path.join(ROOT, "plan-review-design", "SKILL.md");
		if (!fs.existsSync(skill)) return;
		const result = validateSkill(skill);
		expect(result.snapshotFlagErrors).toHaveLength(0);
...
describe("Cross-skill path consistency", () => {
		...
		for (const file of filesToCheck) {
			...
			const hasBoth =
				...
				(content.includes("$REMOTE_SLUG/greptile-history") &&
					content.includes("~/.ship/greptile-history"));
	...
	test("greptile-triage.md contains both project and global history paths", () => {
		...
		expect(content).toContain("~/.ship/greptile-history.md");
	});
...
describe("QA skill structure validation", () => {
	...
	test("output structure references report directory layout", () => {
		...
		expect(qaContent).toContain(".ship/qa-reports/");
	});
});
...
describe("Contributor mode preamble structure", () => {
	...
	for (const skill of skillsWithPreamble) {
		...
		test(`${skill} uses periodic reflection (not per-command)`, () => {
			...
			expect(content).not.toContain("After you use ship-provided CLIs");
		});
	}
});
...
describe("ship-slug", () => {
	const SLUG_BIN = path.join(ROOT, "bin", "ship-slug");
...
describe("Phase 8e.5 regression test generation", () => {
	...
	test("regression test includes full attribution comment format", () => {
		...
		expect(content).toContain("// Report: .ship/qa-reports/");
	});
