...
describe("selectTests", () => {
	test("browse/src change selects browse and qa tests", () => {
		...
		expect(result.selected).not.toContain("plan-review-founder");
	...
	test("skill-specific change selects only that skill and related tests", () => {
		...
		expect(result.selected).toContain("plan-review-founder");
		expect(result.selected).toContain("plan-review-founder-selective");
	...
	test("multiple changed files union their selections", () => {
		const result = selectTests(
			["plan-review-founder/SKILL.md", "retro/SKILL.md.tmpl"],
			E2E_TOUCHFILES,
		);
		expect(result.selected).toContain("plan-review-founder");
		expect(result.selected).toContain("plan-review-founder-selective");
