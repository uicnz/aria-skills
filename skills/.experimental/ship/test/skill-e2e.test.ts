...
// agent behavior. See ARIA.md "E2E eval failure blame protocol" for details.
...
// Eval result collector — accumulates test results, writes to ~/.ship-dev/evals/ on finalize
...
if (evalsEnabled) {
	const check = spawnSync(
		...
		[
			"-c",
			'echo "ping" | aria -p --max-turns 1 --output-format stream-json --verbose --dangerously-skip-permissions',
		],
		{
			stdio: "pipe",
			timeout: 30_000,
...
Report whether it worked.`,
		...
		testIfSelected(
			"skillmd-no-local-binary",
			async () => {
				// Create a tmpdir with no browse binary — no local .aria/skills/ship/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
				...
				// On dev machines with ship installed globally, the fallback path
				// ~/.aria/skills/ship/browse/dist/browse exists, so we get READY.
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of ~/.ship/contributor-logs/

Now try this browse command (it will fail — there is no binary at this path):
/nonexistent/path/browse goto https://example.com

This is a ship issue (the browse binary is missing/misconfigured).
File a contributor report about this issue. Then tell me what you filed.`,
				...
				expect(logContent).toContain("Hey ship team");
		...
		testIfSelected(
			...
			async () => {
				...
				const result = await runSkillTest({
					prompt: `You are running a ship skill. The session preamble detected _SESSIONS=4 (the user has 4 ship windows open).
...
describeIfSelected("Plan CEO Review E2E", ["plan-review-founder"], () => {
	let planDir: string;
...
`,
		...
		// Copy plan-review-founder skill
		fs.mkdirSync(path.join(planDir, "plan-review-founder"), {
			recursive: true,
		});
		fs.copyFileSync(
			path.join(ROOT, "plan-review-founder", "SKILL.md"),
			path.join(planDir, "plan-review-founder", "SKILL.md"),
		);
	});
	...
	test("/plan-review-founder produces structured review output", async () => {
		const result = await runSkillTest({
			prompt: `Read plan-review-founder/SKILL.md for the review workflow.
...
Focus on reviewing the plan content: architecture, error handling, security, and performance.`,
			...
			maxTurns: 15,
			timeout: 360_000,
			testName: "plan-review-founder",
			runId,
		});

		logCost("/plan-review-founder", result);
		recordE2E("/plan-review-founder", "Plan CEO Review E2E", result, {
			passed: ["success", "error_max_turns"].includes(result.exitReason),
		});
...
`,
		...
		test("/plan-review-founder SELECTIVE EXPANSION produces structured review output", async () => {
			const result = await runSkillTest({
				prompt: `Read plan-review-founder/SKILL.md for the review workflow.
...
Focus on reviewing the plan content: architecture, error handling, security, and performance.`,
				...
				maxTurns: 15,
				timeout: 360_000,
				testName: "plan-review-founder-selective",
				runId,
			});

			logCost("/plan-review-founder (SELECTIVE)", result);
			recordE2E(
				"/plan-review-founder-selective",
				"Plan CEO Review SELECTIVE EXPANSION E2E",
				result,
...
describeIfSelected("Plan Eng Review E2E", ["plan-review-eng"], () => {
	let planDir: string;
...
`,
		...
		// Copy plan-review-eng skill
		fs.mkdirSync(path.join(planDir, "plan-review-eng"), { recursive: true });
		fs.copyFileSync(
			path.join(ROOT, "plan-review-eng", "SKILL.md"),
			path.join(planDir, "plan-review-eng", "SKILL.md"),
		);
	});
	...
	test("/plan-review-eng produces structured review output", async () => {
		const result = await runSkillTest({
			prompt: `Read plan-review-eng/SKILL.md for the review workflow.
...
Focus on architecture, code quality, tests, and performance sections.`,
			...
			maxTurns: 15,
			timeout: 360_000,
			testName: "plan-review-eng",
			runId,
		});

		logCost("/plan-review-eng", result);
		recordE2E("/plan-review-eng", "Plan Eng Review E2E", result, {
			passed: ["success", "error_max_turns"].includes(result.exitReason),
		});
		expect(["success", "error_max_turns"]).toContain(result.exitReason);
...
`,
			...
			// Set up remote-slug shim and browse shims (plan-review-eng uses remote-slug for artifact path)
			setupBrowseShims(planDir);

			// Create project directory for artifacts
			projectDir = path.join(os.homedir(), ".ship", "projects", "test-project");
		...
		test("/plan-review-eng writes test-plan artifact to ~/.ship/projects/", async () => {
...
Write your review to ${planDir}/review-output.md`,
				...
				timeout: 360_000,
				testName: "plan-review-eng-artifact",
				runId,
			});

			logCost("/plan-review-eng artifact", result);
			recordE2E(
				"/plan-review-eng test-plan artifact",
				"Plan-Eng-Review Test-Plan Artifact E2E",
				result,
				{
					passed: ["success", "error_max_turns"].includes(result.exitReason),
...
Do NOT push, create PRs, or modify package.json/CHANGELOG.
...
describeIfSelected("Document-Release skill E2E", ["document-release"], () => {
	...
	beforeAll(() => {
		...
		// Create package.json version (already bumped)
		fs.writeFileSync(
			path.join(docReleaseDir, "package.json"),
			JSON.stringify(
				{
...
- Skip package.json version bump (it's already bumped).
...
describeE2E("Deferred skill E2E", () => {
	// Ship is destructive: pushes to remote, creates PRs, modifies package.json/CHANGELOG
...
describeIfSelected("ship-upgrade E2E", ["ship-upgrade-happy-path"], () => {
	let upgradeDir: string;
	let remoteDir: string;

	beforeAll(() => {
		upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), "skill-e2e-upgrade-"));
		remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), "ship-remote-"));
		...
		// Create mock ship install directory (local-git type)
		const mockShip = path.join(upgradeDir, ".aria", "skills", "ship");
		fs.mkdirSync(mockShip, { recursive: true });

		// Init as a git repo
		run("git", ["init"], mockShip);
		run("git", ["config", "user.email", "test@test.com"], mockShip);
		run("git", ["config", "user.name", "Test"], mockShip);

		// Create bare remote
		run("git", ["init", "--bare"], remoteDir);
		run("git", ["remote", "add", "origin", remoteDir], mockShip);

		// Write old version files
		fs.writeFileSync(
			path.join(mockShip, "package.json"),
			JSON.stringify(
				{
					name: "ship",
					version: "0.5.0",
		...
		fs.writeFileSync(
			path.join(mockShip, "CHANGELOG.md"),
			"# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n",
		);
		fs.writeFileSync(
			path.join(mockShip, "setup"),
			'#!/bin/bash\necho "Setup completed"\n',
			{ mode: 0o755 },
		);

		// Initial commit + push
		run("git", ["add", "."], mockShip);
		run("git", ["commit", "-m", "initial"], mockShip);
		run("git", ["push", "-u", "origin", "HEAD:main"], mockShip);
		...
		run("git", ["add", "."], mockShip);
		run("git", ["commit", "-m", "release 0.6.0"], mockShip);
		run("git", ["push", "origin", "HEAD:main"], mockShip);

		// Reset working copy back to old version
		run("git", ["reset", "--hard", "HEAD~1"], mockShip);

		// Copy ship-upgrade skill
		fs.mkdirSync(path.join(upgradeDir, "ship-upgrade"), { recursive: true });
		fs.copyFileSync(
			path.join(ROOT, "ship-upgrade", "SKILL.md"),
			path.join(upgradeDir, "ship-upgrade", "SKILL.md"),
		);
	...
	testIfSelected(
		"ship-upgrade-happy-path",
		async () => {
			const mockShip = path.join(upgradeDir, ".aria", "skills", "ship");
			const result = await runSkillTest({
				prompt: `Read ship-upgrade/SKILL.md for the upgrade workflow.

You are running /ship-upgrade standalone. The ship installation is at ./.aria/skills/ship (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.aria/skills/ship — use that exact path.`,
				workingDirectory: upgradeDir,
				maxTurns: 20,
				timeout: 180_000,
				testName: "ship-upgrade-happy-path",
				runId,
			});

			logCost("/ship-upgrade happy path", result);

			// Check that the version was updated
			const versionAfter = JSON.parse(
			...
			recordE2E("/ship-upgrade happy path", "ship-upgrade E2E", result, {
				passed:
					versionAfter === "0.6.0" &&
...
Write DESIGN.md and ARIA.md (or update it) in the working directory.`,
					workingDirectory: designDir,
					maxTurns: 20,
					timeout: 360_000,
				...
				const ariaPath = path.join(designDir, "ARIA.md");
				const designExists = fs.existsSync(designPath);
				const ariaExists = fs.existsSync(ariaPath);
				let designContent = "";
				...
				const structuralPass =
					designExists && ariaExists && missingSections.length === 0;
				recordE2E(
					"/design-consultation core",
					"Design Consultation E2E",
					result,
				...
				if (ariaExists) {
					const aria = fs.readFileSync(ariaPath, "utf-8");
					expect(aria.toLowerCase()).toContain("design.md");
				}
			},
			420_000,
		);
		...
		testIfSelected(
			...
			async () => {
				...
				try {
					fs.unlinkSync(path.join(designDir, "ARIA.md"));
				} catch {}
...
describeIfSelected(
	...
	["plan-review-design-plan-mode", "plan-review-design-no-ui-scope"],
	() => {
		let reviewDir: string;
		...
		beforeAll(() => {
			...
			// Copy plan-review-design skill
			fs.mkdirSync(path.join(reviewDir, "plan-review-design"), {
				recursive: true,
			});
			fs.copyFileSync(
				path.join(ROOT, "plan-review-design", "SKILL.md"),
				path.join(reviewDir, "plan-review-design", "SKILL.md"),
			);
...
`,
		...
		testIfSelected(
			"plan-review-design-plan-mode",
			async () => {
				const result = await runSkillTest({
					prompt: `Read plan-review-design/SKILL.md for the design review workflow.
...
IMPORTANT: Do NOT try to browse any URLs or use a browse binary. This is a plan review, not a live site audit. Just read the plan file, review it, and edit it to fix the gaps.`,
					...
					maxTurns: 15,
					timeout: 300_000,
					testName: "plan-review-design-plan-mode",
					runId,
				});

				logCost("/plan-review-design plan-mode", result);
		...
		testIfSelected(
			"plan-review-design-no-ui-scope",
			async () => {
...
IMPORTANT: Do NOT try to browse any URLs or use a browse binary. This is a plan review, not a live site audit.`,
					...
					maxTurns: 10,
					timeout: 180_000,
					testName: "plan-review-design-no-ui-scope",
					runId,
				});

				logCost("/plan-review-design no-ui-scope", result);
