 ...
 *   2. git rev-parse --show-toplevel → projectDir/.ship/
 *
...
export function resolveConfig(
	...
	if (env.BROWSE_STATE_FILE) {
		...
		projectDir = path.dirname(stateDir); // parent of .ship/
	} else {
		const gitRoot = getGitRoot();
		projectDir = gitRoot || process.cwd();
		stateDir = path.join(projectDir, ".ship");
...
/**
 * Create the .ship/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
	...
	// Ensure .ship/ is in the project's .gitignore
	const gitignorePath = path.join(config.projectDir, ".gitignore");
	try {
		const content = fs.readFileSync(gitignorePath, "utf-8");
		if (!content.match(/^\.ship\/?$/m)) {
			const separator = content.endsWith("\n") ? "" : "\n";
			fs.appendFileSync(gitignorePath, `${separator}.ship/\n`);
		}
	} catch (err: any) {
		if (err.code !== "ENOENT") {
 ...
 * If no remote is configured, use the repository directory basename.
 */
