...
export class BrowserManager {
	...
	async launch() {
		...
		this.browser.on("disconnected", () => {
			...
			console.error(
				"[browse] Console/network logs flushed to .ship/browse-*.log",
			);
			process.exit(1);
		});
	...
	async recreateContext(): Promise<string | null> {
		...
		try {
			...
			// Recovery path: create a clean context + blank tab
			try {
				this.pages.clear();
				...
				// If even recovery fails, browser stays alive but context reset failed
			}
	...
	private wirePageEvents(page: Page) {
		...
		page.on("response", (res) => {
			// Find matching request entry and update it (reverse scan)
			const url = res.url();
