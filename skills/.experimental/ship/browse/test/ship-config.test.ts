/**
 * Tests for bin/ship-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * SHIP_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, "..", "..", "bin", "ship-config");
...
beforeEach(() => {
	stateDir = mkdtempSync(join(tmpdir(), "ship-config-test-"));
});
