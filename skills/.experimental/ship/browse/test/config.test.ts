...
describe("config", () => {
	...
	describe("resolveConfig", () => {
		test("uses git root by default", () => {
			...
			expect(config.stateDir).toBe(path.join(gitRoot!, ".ship"));
			expect(config.stateFile).toBe(
				path.join(gitRoot!, ".ship", "browse.json"),
			);
		});

		test("derives paths from BROWSE_STATE_FILE when set", () => {
			const stateFile = "/tmp/test-config/.ship/browse.json";
			const config = resolveConfig({ BROWSE_STATE_FILE: stateFile });
			expect(config.stateFile).toBe(stateFile);
			expect(config.stateDir).toBe("/tmp/test-config/.ship");
	...
	describe("ensureStateDir", () => {
		test("creates directory if it does not exist", () => {
			...
			const config = resolveConfig({
				BROWSE_STATE_FILE: path.join(tmpDir, ".ship", "browse.json"),
			});
		...
		test("is a no-op if directory already exists", () => {
			const tmpDir = path.join(os.tmpdir(), `browse-config-test-${Date.now()}`);
			const stateDir = path.join(tmpDir, ".ship");
		...
		test("adds .ship/ to .gitignore if not present", () => {
		...
		test("does not duplicate .ship/ in .gitignore", () => {
			const tmpDir = path.join(
				os.tmpdir(),
				`browse-gitignore-test-${Date.now()}`,
			);
		...
		test("logs warning to browse-server.log on non-ENOENT gitignore error", () => {
			...
			fs.mkdirSync(tmpDir, { recursive: true });
			// Create a read-only .gitignore (no .ship/ entry → would try to append)
			fs.writeFileSync(path.join(tmpDir, ".gitignore"), "node_modules/\n");
	...
	describe("getRemoteSlug", () => {
		...
		test("parses SSH remote URLs", () => {
			...
			const url = "git@github.com:shaneholloman/ship.git";
			const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
			expect(match).not.toBeNull();
			expect(`${match![1]}-${match![2]}`).toBe("shaneholloman-ship");
		});

		test("parses HTTPS remote URLs", () => {
			const url = "https://github.com/shaneholloman/ship.git";
			const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
			expect(match).not.toBeNull();
			expect(`${match![1]}-${match![2]}`).toBe("shaneholloman-ship");
		});
		...
		test("parses HTTPS remote URLs without .git suffix", () => {
			const url = "https://github.com/shaneholloman/ship";
			const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
			expect(match).not.toBeNull();
			expect(`${match![1]}-${match![2]}`).toBe("shaneholloman-ship");
		});
