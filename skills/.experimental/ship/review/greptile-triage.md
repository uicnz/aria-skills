...
```sh
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.aria/skills/ship/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.ship/projects/$REMOTE_SLUG/greptile-history.md"
```
...
Use Tier 2 when escalation detection (below) identifies a prior Ship reply on the same thread. Include maximum evidence to close the discussion.
...
Before composing a reply, check if a prior Ship reply already exists on this comment thread:

1. **For line-level comments:** Fetch replies via `gh api repos/$REPO/pulls/$PR_NUMBER/comments/$COMMENT_ID/replies`. Check if any reply body contains Ship markers: `**Fixed**`, `**Not a bug.**`, `**Already fixed**`.

2. **For top-level comments:** Scan the fetched issue comments for replies posted after the Greptile comment that contain Ship markers.

3. **If a prior Ship reply exists AND Greptile posted again on the same file+category:** Use Tier 2 (firm) templates.

4. **If no prior Ship reply exists:** Use Tier 1 (friendly) templates.
...
```sh
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.aria/skills/ship/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.ship/projects/$REMOTE_SLUG"
mkdir -p ~/.ship
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):

- `~/.ship/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.ship/greptile-history.md` (global aggregate)
...
```
2026-03-13 | shaneholloman/myapp | fp | app/services/auth_service.rb | race-condition
2026-03-13 | shaneholloman/myapp | fix | app/models/user.rb | null-check
2026-03-13 | shaneholloman/myapp | already-fixed | lib/payments.rb | error-handling
```
