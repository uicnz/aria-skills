...
Then read ARIA.md, TODOS.md, and any existing architecture docs. When reading TODOS.md, specifically:
...
mkdir -p ~/.ship/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```sh
mkdir -p ~/.ship/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.ship/projects/$SLUG/ceo-plans/{old-plan}.md ~/.ship/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.ship/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
Generated by /plan-review-founder on {date}
...
NOTE: These represent human-team implementation hours. With CC + ship,
...
For LLM/prompt changes: Check ARIA.md for the "Prompt/LLM changes" file patterns. If this plan touches ANY of those patterns, state which eval suites must be run, which cases should be added, and what baselines to compare against.
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
The CEO calling in the designer. Not a pixel-level audit — that's /plan-review-design and /design-review. This is ensuring the plan has design intentionality.
...
If this plan has significant UI scope, recommend: "Consider running /plan-review-design for a deep design review of this plan before implementation."
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
Present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `.aria/skills/review/TODOS-format.md`.
...
- **Effort estimate:** S/M/L/XL (human team) → with CC+ship: S→S, M→S, L→M, XL→L
- **Priority:** P1/P2/P3
...
echo '{"skill":"plan-review-founder","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE"}' >> ~/.ship/projects/$SLUG/$BRANCH-reviews.jsonl
```
...
- **B)** Keep in `~/.ship/projects/` only (local, personal reference)
- **C)** Skip
...
| Dimension              | Expansion              | Selective Expansion                    | Hold Scope             | Reduction                    |
| ---------------------- | ---------------------- | -------------------------------------- | ---------------------- | ---------------------------- |
| Scope                  | Push up (opt-in)       | Hold + offer                           | Maintain               | Push down                    |
| Recommend posture      | Enthusiastic           | Neutral                                | N/A                    | N/A                          |
| 10x check              | Mandatory              | Surface as cherry-pick                 | Optional               | Skip                         |
| Platonic ideal         | Yes                    | No                                     | No                     | No                           |
| Delight opportunities  | Opt-in ceremony        | Cherry-pick ceremony                   | Note if seen           | Skip                         |
| Complexity question    | "Is it big enough?"    | "Is it right + what else is tempting?" | "Is it too complex?"   | "Is it the bare minimum?"    |
| Taste calibration      | Yes                    | Yes                                    | No                     | No                           |
| Temporal interrogation | Full (hr 1-6)          | Full (hr 1-6)                          | Key decisions only     | Skip                         |
| Observability standard | "Joy to operate"       | "Joy to operate"                       | "Can we debug it?"     | "Can we see if it's broken?" |
| Deploy standard        | Infra as feature scope | Safe deploy + cherry-pick risk check   | Safe deploy + rollback | Simplest possible deploy     |
| Error map              | Full + chaos scenarios | Full + chaos for accepted              | Full                   | Critical paths only          |
| CEO plan               | Written                | Written                                | Skipped                | Skipped                      |
| Phase 2/3 planning     | Map accepted           | Map accepted cherry-picks              | Note it                | Skip                         |
