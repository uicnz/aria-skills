...
**What:** Append structured JSON entry to `.ship/ship-log.json` at end of every /ship run (version, date, branch, PR URL, review findings, Greptile stats, todos completed, test results).
...
**Effort:** L
**Priority:** P2
**Depends on:** /setup-ship-upload, visual PR annotations
...
**Why:** Line-level annotations are more actionable than top-level comments. The PR thread becomes a line-by-line conversation between Greptile, Aria, and human reviewers.
...
### /setup-ship-upload skill (S3 bucket)
...
**What:** `browse/bin/ship-upload` — upload file to S3, return public URL.
...
**Context:** Requires `ANTHROPIC_API_KEY` in CI secrets. Cost is ~$4/run. Eval persistence system (v0.3.6) writes JSON to `~/.ship-dev/evals/` — CI would upload as GitHub Actions artifacts and use `eval:compare` to post delta comment.
...
**What:** Pin E2E tests to aria-sonnet-4-6 for cost efficiency, add retry:2 for flaky LLM responses.
...
**Context:** Reads `~/.ship-dev/evals/*.json`. ~200 lines HTML + chart.js via Bun HTTP server.
...
**Context:** Requires headless browse binary available in CI. The `/qa` skill already produces `baseline.json` with health scores — CI step would compare against the main branch baseline and fail if score drops. Would need `ANTHROPIC_API_KEY` in CI secrets since `/qa` uses Aria.
...
**What:** `ship-open-url` helper script — detect platform, use `open` (macOS) or `xdg-open` (Linux).

**Why:** The first-time Completeness Principle intro uses macOS `open` to launch the essay. If ship ever supports Linux, this silently fails.
...
Shipped as `/design-consultation` on shaneholloman/design branch. Renamed from `/setup-design-md` to reflect the consultant approach (agent proposes a complete coherent system, user adjusts). Includes competitive research via WebSearch, combined font+color preview page, coherence validation, and LLM-judged E2E tests.
...
**What:** Create a placeholder resolver in gen-skill-docs.ts encoding the ship voice guide (friendly, user-forward, lead with benefits). Inject into /ship Step 5, /document-release Step 5, and reference from ARIA.md.

**Why:** DRY — voice rules currently live inline in 3 places (ARIA.md CHANGELOG style section, /ship Step 5, /document-release Step 5). When the voice evolves, all three drift.
...
`bin/ship-diff-scope` shipped — categorizes diff into SCOPE_FRONTEND, SCOPE_BACKEND, SCOPE_PROMPTS, SCOPE_TESTS, SCOPE_DOCS, SCOPE_CONFIG. Used by design-review-lite to skip when no frontend files changed. Dashboard integration for conditional row display is a follow-up.
...
**Effort:** S
**Priority:** P3
**Depends on:** ship-diff-scope (shipped)
...
**What:** Track how often Aria chooses the complete option vs shortcut across ship sessions. Aggregate into a dashboard showing completeness trend over time.
...
**Effort:** M (human) / S (CC)
**Priority:** P3
**Depends on:** Boil the Lake shipped
...
- Config CLI (`bin/ship-config`), auto-upgrade via `~/.ship/config.yaml`, 12h cache TTL, exponential snooze backoff (24h→48h→1wk), "never ask again" option, vendored copy sync on upgrade
  **Completed:**
