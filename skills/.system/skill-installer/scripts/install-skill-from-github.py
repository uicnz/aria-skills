#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $ARIA_HOME/skills."""
...
def _aria_home() -> str:
    return os.environ.get("ARIA_HOME", os.path.expanduser("~/.aria"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "aria")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "aria-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_aria_home(), "skills")
