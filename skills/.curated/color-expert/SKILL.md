---
name: color-expert
description: Use when a task requires substantive color expertise—such as naming, theory, color-space selection or conversion, palette construction, accessibility, perceptual matching, pigment mixing, print-versus-screen behavior, CSS color standards, or historical terminology—not for routine design or code changes that merely use colors.
---
...
A comprehensive knowledge base covering color naming, theory, spaces, and definitions; palettes, ramps, and gradients; conversions, accessibility, and perceptual matching; pigment mixing and print-versus-screen behavior; CSS color syntax; and historical terminology. See `references/INDEX.md` for 100+ detailed reference files; this skill file contains the essential knowledge to answer most questions directly.

## Workflow

1. Confirm that the task needs color expertise rather than routine styling or code changes that happen to contain color values.
2. Identify the medium and constraints: screen, print, paint, video, or HDR; target gamut and color profile; viewing conditions; accessibility target; existing brand or design tokens; and required output format.
3. Start with the compact guidance in this file and avoid loading deep references when it already answers the question.
4. For specialist or source-sensitive questions, use the reference routing table below, then open only the relevant files; use `references/INDEX.md` to discover additional sources.
5. For implementation work, inspect the project's existing color tokens and conventions before proposing changes, preserve semantic roles, and validate conversions or contrast with deterministic tooling when available.
6. Verify volatile claims—such as browser support, standards status, library APIs, or current tool availability—against current primary sources before relying on the static references.
7. State assumptions and distinguish standards, measured facts, implementation constraints, heuristics, and aesthetic judgment.

## Reference Routing

| Need | Read first |
| --- | --- |
| Color terminology and appearance | `references/contemporary/chroma-vs-saturation.md`, `references/contemporary/lightness-vs-brightness.md`, `references/contemporary/ciecam02-color-appearance-model.md` |
| Color spaces, CSS, and conversion | `references/techniques/w3c-css-color-4-and-5.md`, `references/techniques/culori-color-spaces-api.md`, `references/techniques/hsluv-better-than-hsl.md` |
| Accessibility and contrast | `references/techniques/apca-myndex-contrast.md`, `references/techniques/color-buddy-palette-lint.md`, `references/contemporary/accessible-color-combinations-count.md` |
| Palette ramps and harmony | `references/techniques/rampensau-palette-generation.md`, `references/techniques/poline-esoteric-palette-generator.md`, `references/techniques/youre-wrong-about-color-harmony.md` |
| Pigment and paint mixing | `references/contemporary/huevaluechroma/ch06-mixing-of-paints.md`, `references/techniques/spectraljs-pigment-mixing.md`, `references/techniques/imaginary-pigments-mixbox.md` |
| Print and screen matching | `references/techniques/icc-profile-color-management.md`, `references/techniques/screen-to-print-colour-fidelity.md` |
| Color naming | `references/techniques/color-name-lists.md`, `references/techniques/color-description-library.md`, `references/historical/ridgway-color-standards-1912.md` |
| Historical color systems | Open the `Historical` section of `references/INDEX.md`, then load only the matching source notes. |

## Output Rules

- Lead with the practical recommendation, then explain the color-science reason at the depth the task needs.
- Specify the color space, white point, gamut, and alpha interpretation whenever numeric color coordinates could be ambiguous.
- Treat contrast and gamut as properties to calculate or test, not as visual guesses.
- Separate normative requirements from useful heuristics and subjective palette advice.
- Cite the original source links recorded in the references when attribution or evidence matters; paraphrase long source material rather than reproducing it.
- Do not dump the reference library into the response; synthesize only what answers the task.
...
Treat these as candidate tools and verify current availability, package APIs, and platform support before recommending or installing them.
