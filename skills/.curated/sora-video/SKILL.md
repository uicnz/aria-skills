---
name: sora-video
description: 'Use when the user wants to generate, edit, extend, manage, or download Sora videos, create reusable non-human character references, or plan or run multiple Sora jobs.'
---
...
Create or manage Sora video jobs for the current project, including product demos, marketing spots, cinematic shots, social clips, and UI mocks. Default to `sora-2` with structured prompt augmentation and prefer the bundled `scripts/sora.py` CLI for deterministic runs. Live calls require `OPENAI_API_KEY` and Sora API access; `$sora-video` is a skill tag in prompts, not a shell command.
...
- If the user needs many renders immediately inside Aria → `create-batch` (local fan-out, not the Batch API)
...
- **`references/aria-network.md`**: network/approval troubleshooting.
