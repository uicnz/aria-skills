...
export SORA_CLI="$ARIA_HOME/skills/sora-video/scripts/sora.py"
```
...
```bash
mkdir -p tmp/sora-video
cat > tmp/sora-video/prompts.jsonl << 'EOB'
...
uv run --with openai python "$SORA_CLI" create-batch \
  --input tmp/sora-video/prompts.jsonl \
  --out-dir out \
  --concurrency 3
```
