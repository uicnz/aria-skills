---
name: web-game
description: 'Use when building or iterating on an HTML or JavaScript web game.'
---
...
Use the Playwright-based test client with short input bursts and intentional pauses, inspect screenshots and `render_game_to_text` output, and review console errors after each meaningful change.

## Skill paths (set once)

```bash
export ARIA_HOME="${ARIA_HOME:-$HOME/.aria}"
export WEB_GAME_CLIENT="$ARIA_HOME/skills/web-game/scripts/web_game_playwright_client.js"
export WEB_GAME_ACTIONS="$ARIA_HOME/skills/web-game/references/action_payloads.json"
```

User-scoped skills install under `$ARIA_HOME/skills` (default: `~/.aria/skills`).
...
Create a `progress.md` file if it doesn't exist, and append remaining tasks, notes, gotchas, and loose ends as you go so another agent can pick up seamlessly.
...
At the end of your work, leave remaining tasks and suggestions for the next agent in `progress.md`.
...
- `$WEB_GAME_CLIENT` (installed default: `$ARIA_HOME/skills/web-game/scripts/web_game_playwright_client.js`) — Playwright-based action loop with virtual-time stepping, screenshot capture, and console error buffering. You must pass an action burst via `--actions-file`, `--actions-json`, or `--click`.

## References

- `$WEB_GAME_ACTIONS` (installed default: `$ARIA_HOME/skills/web-game/references/action_payloads.json`) — example action payloads (keyboard + mouse, per-frame capture). Use these to build your burst.
