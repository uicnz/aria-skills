...
description: 'Use when the user wants to inspect Sentry issues or events, summarize recent production errors, or retrieve basic Sentry health data.'
---
...
Perform read-only Sentry API queries with the bundled `scripts/sentry_api.py` helper. Live queries require `SENTRY_AUTH_TOKEN` with appropriate read-only scopes.
...
export SENTRY_API="$ARIA_HOME/skills/sentry-observability/scripts/sentry_api.py"
```
