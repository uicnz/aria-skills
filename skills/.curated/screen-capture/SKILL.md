---
name: screen-capture
description: 'Use when the user asks for an OS-level screenshot of a screen, app, window, or region, or when a more specific capture tool is unavailable.'
---
...
Capture full screens, specific apps or windows, and pixel regions with the bundled operating-system helpers, while preferring tool-specific capture capabilities when they are available.
...
The helpers route Swift's module cache to `$TMPDIR/aria-swift-module-cache`
...
```bash
bash <path-to-skill>/scripts/ensure_macos_permissions.sh && \
python3 <path-to-skill>/scripts/take_screenshot.py --app "Aria"
```

For Aria inspection runs, keep the output in temp:
...
- Temp location (Aria visual check):
...
```bash
python3 <path-to-skill>/scripts/take_screenshot.py --app "Aria" --window-name "Settings"
```

- List matching window ids before capturing (macOS only):

```bash
python3 <path-to-skill>/scripts/take_screenshot.py --list-windows --app "Aria"
```
