#!/usr/bin/env python3
"""Cross-platform screenshot helper for Aria skills."""
...
TEST_MODE_ENV = "ARIA_SCREENSHOT_TEST_MODE"
TEST_PLATFORM_ENV = "ARIA_SCREENSHOT_TEST_PLATFORM"
TEST_WINDOWS_ENV = "ARIA_SCREENSHOT_TEST_WINDOWS"
TEST_DISPLAYS_ENV = "ARIA_SCREENSHOT_TEST_DISPLAYS"
TEST_PNG = (
...
def resolve_output_path(
    ...
    if mode == "temp":
        ...
        tmp_path = tmp_dir / default_filename(fmt, prefix="aria-shot")
...
def swift_json(script: Path, extra_args: list[str] | None = None) -> dict:
    module_cache = Path(tempfile.gettempdir()) / "aria-swift-module-cache"
...
def ensure_macos_permissions() -> None:
    if os.environ.get("ARIA_SANDBOX"):
        raise SystemExit(
