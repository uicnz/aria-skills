...
description: 'Use when the user wants to generate, edit, inpaint, composite, remove or replace backgrounds, or create variants of raster images with the OpenAI Image API.'
---
...
Generate or edit images for the current project, including website and game assets, UI and product mockups, wireframes, logos, photorealistic images, concept art, covers, and infographics. Default to `gpt-image-1.5` and the OpenAI Image API, prefer the bundled `scripts/image_gen.py` CLI for deterministic and reproducible runs, and require `OPENAI_API_KEY` for live calls.
...
- Use `tmp/image-generation/` for intermediate files (for example JSONL batches); delete when done.
- Write final artifacts under `output/image-generation/` when working in this repo.
...
- If network approvals / sandbox settings are getting in the way: `references/aria-network.md`
...
- **`references/aria-network.md`**: environment/sandbox/network-approval troubleshooting.
