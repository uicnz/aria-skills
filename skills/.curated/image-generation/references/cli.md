...
export IMAGE_GEN="$ARIA_HOME/skills/image-generation/scripts/image_gen.py"
```
...
```
mkdir -p tmp/image-generation
cat > tmp/image-generation/prompts.jsonl << 'EOF'
...
python "$IMAGE_GEN" generate-batch --input tmp/image-generation/prompts.jsonl --out-dir out --concurrency 5

# Cleanup (recommended)
rm -f tmp/image-generation/prompts.jsonl
```
