...
Use this snippet to register the Figma MCP server in `~/.aria/config.toml` as a streamable HTTP server with bearer auth pulled from your env.
...
- The bearer token must be available as `FIGMA_OAUTH_TOKEN` in the environment that launches Aria.
...
- Verify before launching Aria: `echo $FIGMA_OAUTH_TOKEN` should print a non-empty token.

## Setup + verification checklist

- Add the snippet above to `~/.aria/config.toml` under `[mcp_servers.figma]`, and enable `[features].rmcp_client = true` (or `experimental_use_rmcp_client = true` on older releases).
- Restart Aria (CLI/IDE) after updating config and env vars.
- Ask Aria to list Figma tools or run a simple call to confirm the server is reachable.

## Troubleshooting

- Token not picked up: Export `FIGMA_OAUTH_TOKEN` in the same shell that launches Aria, or add it to your shell profile and restart.
